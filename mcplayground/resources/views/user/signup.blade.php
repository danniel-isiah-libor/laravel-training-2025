...
@section('content')
    <x-card-slot>
        ...
        <div class="text-xl mt-4">Signup</div>
        <div class="text-neutral-500 text-[15px]">Playground registration</div>
        ...
        <form method="POST" action="{{ route('signup.store') }}" class="w-full">
            ...
            <div class="flex flex-col gap-2">
                <x-input-component name="name" label="Name" type="text" />
                <x-input-component name="email" label="Email" type="test" />
                <x-input-component name="password" label="Password" type="password" />
                <x-input-component name="password_confirmation" label="Confirm password" type="password" />
                <div class="flex flex-col gap-1 w-full mt-4">
                    <x-submit-button label="Sign Up" />
                </div>
                <div class="flex flex-col justify-center items-center mt-4 text-[15px]">
                    <div>Already have an account?</div>
                    <div><a href="{{ route('signin') }}" class="text-blue-700">Login instead</a></div>
                </div>
            </div>
        </form>
    </x-card-slot>
