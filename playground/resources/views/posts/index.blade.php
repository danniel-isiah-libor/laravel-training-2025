<x-layout>
  ...
  <div class="mx-auto max-w-7xl px-6 lg:px-8">
    <div class="mx-auto max-w-2xl lg:mx-0">
      ...
      <h2 class="text-4xl font-semibold tracking-tight text-pretty  sm:text-5xl bg-gradient-to-r from-pink-500 to-orange-400 ... text-transparent bg-clip-text inline-block">Post Listing</h2>
    <p class="mt-2 text-lg/8 text-gray-300">{{ $posts->links() }}</p>
    </div>
    <div class="mx-auto mt-10 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 border-t border-gray-700 pt-10 sm:mt-16 sm:pt-16 lg:mx-0 lg:max-w-none lg:grid-cols-3">
        ...
        <div class="flex items-center gap-x-4 text-xs">
          {{-- <time datetime="2020-03-16" class="text-gray-400">{{$post->updated_at->format('M d, Y')}}</time> --}}
          <time datetime="2020-03-16" class="text-gray-400">{{$post->updated_at->diffForHumans()}}</time>
